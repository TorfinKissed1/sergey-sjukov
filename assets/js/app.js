...
(function () {
  ...
  function setActiveService(index, progressValue) {
    serviceCards.forEach((card, cardIndex) => {
      ...
      const localProgress = serviceCards.length <= 1
        ? 1
        : clamp((progressValue * (serviceCards.length - 1)) - cardIndex + 1, 0, 1);
  ...
  function updatePinnedServices() {
    ...
    const exactIndex = progressValue * (serviceCards.length - 1);
    const activeIndex = clamp(Math.round(exactIndex), 0, serviceCards.length - 1);
  ...
  serviceNavItems.forEach((item) => {
    item.addEventListener('click', () => {
      ...
      const maxDistance = Math.max(1, servicesSection.offsetHeight - window.innerHeight);
      const targetY = servicesSection.offsetTop + maxDistance * (index / (serviceCards.length - 1));
  ...
  function applyScrollState() {
    ...
    if (progress) {
      ...
      progress.style.setProperty('--scroll', (scrollY / maxScroll) * 100 + '%');
    }
